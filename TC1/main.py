...
for test_size in TESTS_SIZE:
    ...
    print(df.describe())
acc_acumulator=[]
def CentroidClassifier(method='classic',acc_cumulator = [],test_sizes=TESTS_SIZE, EPOCHS = 100) -> pd.DataFrame:
    """
    ...
    Plota as matrizes de confusao e guarda todas as estatisticas e imagens geradas em results
    """
    cc = centroidClassifier(method=method)
    for test_size in test_sizes:
        ...
        axs[1].set_title(f"Worst epoch confusion matrix - acc:{worst[0]:.4f}")
        os.makedirs(f"results/confusion_matrix_{t_size:.0f}_{train_size:.0f}", exist_ok=True)
        plt.savefig(f"results/confusion_matrix_{t_size:.0f}_{train_size:.0f}/best_worst_centroid_{method}_{0.5 if method != 'mahala' else 'quadratic'}_{EPOCHS}_epochs.png",dpi=150,bbox_inches='tight')
        plt.close()
...
CentroidClassifier()
acc_acumulator2=[]
#CENTROIDE ROBUSTO A OUTLIER
CentroidClassifier(acc_cumulator=acc_acumulator2,method='outlier robust')
#CENTROIDE COM DISTANCIA DE MAHALA - MAXIMA CORRELAÇÃO
acc_acumulator3=[]
